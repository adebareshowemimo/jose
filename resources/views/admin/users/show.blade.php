...
@section('content')
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @if($user->candidate && ! empty($user->candidate->awards))
                <div class="bg-white rounded-xl border border-gray-200 p-6">
                    <h3 class="text-base font-semibold text-gray-900 mb-4">Certifications</h3>
                    <ul class="space-y-3">
                        @foreach($user->candidate->awards as $cert)
                            <li class="flex items-start justify-between gap-3 border-b border-gray-100 pb-3 last:border-0 last:pb-0">
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs text-gray-500">
                                        ...
                                        @if(! empty($cert['issue_date'])) · {{ $cert['issue_date'] }}@endif
                                        @if(! empty($cert['expiry_date'])) – Expires {{ $cert['expiry_date'] }} @elseif(! empty($cert['no_expiry'])) · No expiry @endif
