...
@php
    $isEmployerChat = $selectedConversation && $selectedConversation->type === \App\Models\ChatConversation::TYPE_ADMIN_EMPLOYER;
    $headerTitle = $isEmployerChat
        ? ($selectedConversation->company?->name ?? 'Employer')
        : ($selectedConversation?->candidate?->user?->name ?? 'Candidate');
    $headerSub = $isEmployerChat
        ? ($selectedConversation->company?->email ?? $selectedConversation->company?->owner?->email)
        : ($selectedConversation?->candidate?->user?->email);
@endphp

<div class="grid gap-6 xl:grid-cols-[360px_1fr]" x-data="{ tab: '{{ $activeTab }}' }">
    <div class="space-y-6">
        <div class="rounded-xl border border-gray-200 bg-white p-2 flex gap-1">
            <button type="button" @click="tab = 'candidates'"
                    :class="tab === 'candidates' ? 'bg-[#073057] text-white' : 'text-gray-600 hover:bg-gray-50'"
                    class="flex-1 px-3 py-2 text-sm font-semibold rounded-lg transition">
                Candidates
            </button>
            <button type="button" @click="tab = 'employers'"
                    :class="tab === 'employers' ? 'bg-[#073057] text-white' : 'text-gray-600 hover:bg-gray-50'"
                    class="flex-1 px-3 py-2 text-sm font-semibold rounded-lg transition">
                Employers
            </button>
        </div>
        ...
        {{-- Candidates panel --}}
        <div x-show="tab === 'candidates'" class="space-y-6">
            ...
            <div class="rounded-xl border border-gray-200 bg-white overflow-hidden">
                <div class="p-4 border-b border-gray-200">
                    <h2 class="font-semibold text-gray-900">Candidate Conversations</h2>
                </div>
                <div class="max-h-[460px] overflow-y-auto">
                    @forelse($candidateConversations as $conversation)
                        @php
                    ...
                    @empty
                        <p class="text-sm text-gray-400 text-center py-8">No candidate chats yet.</p>
                    @endforelse
                </div>
            </div>
        </div>
        ...
        {{-- Employers panel --}}
        <div x-show="tab === 'employers'" x-cloak class="space-y-6">
            <div class="rounded-xl border border-gray-200 bg-white p-4"
                 x-data="{
                     q: '',
                     results: [],
                     ...
                     async fetch() {
                         this.loading = true;
                         try {
                             const url = new URL('{{ route('admin.chat.employers.search') }}', window.location.origin);
                             if (this.q) url.searchParams.set('q', this.q);
                             const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
                             const data = await res.json();
                             this.results = data.results || [];
                     ...
                     gotoCompany(id) {
                         window.location.href = '{{ route('admin.chat.index') }}?company_id=' + id;
                     }
                 }">
                <h2 class="font-semibold text-gray-900 mb-3">Start Employer Chat</h2>
                <div class="relative mb-3">
                    <input type="text" x-model="q" @input="onInput()" placeholder="Type a company or email..."
                        class="w-full px-3 py-2 pr-9 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#1AAD94] focus:border-transparent">
                    <span x-show="loading" class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400">…</span>
                </div>
                <div class="space-y-2 max-h-72 overflow-y-auto">
                    <template x-for="c in results" :key="c.id">
                        <button type="button" @click="gotoCompany(c.id)"
                                class="w-full text-left rounded-lg border border-gray-100 p-3 hover:bg-gray-50 transition">
                            <p class="text-sm font-semibold text-gray-900" x-text="c.name"></p>
                            <p class="text-xs text-gray-500" x-text="c.email || c.owner_name"></p>
                        </button>
                    </template>
                    <p x-show="hasSearched && !loading && results.length === 0"
                       class="text-sm text-gray-400 text-center py-6"
                       x-text="q ? 'No matches for &quot;' + q + '&quot;.' : 'No employers found.'"></p>
                </div>
            </div>

            <div class="rounded-xl border border-gray-200 bg-white overflow-hidden">
                <div class="p-4 border-b border-gray-200">
                    <h2 class="font-semibold text-gray-900">Employer Conversations</h2>
                </div>
                <div class="max-h-[460px] overflow-y-auto">
                    @forelse($employerConversations as $conversation)
                        @php
                            $companyName = $conversation->company?->name ?? 'Company';
                            $unread = $conversation->messages()->where('sender_user_id', '!=', auth()->id())->whereNull('read_at')->count();
                        @endphp
                        <a href="{{ route('admin.chat.index', ['conversation' => $conversation->id]) }}" class="flex gap-3 p-4 border-b border-gray-100 hover:bg-gray-50 {{ $selectedConversation?->id === $conversation->id ? 'bg-[#1AAD94]/5' : '' }}">
                            <div class="w-10 h-10 rounded-full bg-[#073057]/10 text-[#073057] flex items-center justify-center font-semibold">{{ strtoupper(mb_substr($companyName, 0, 1)) }}</div>
                            <div class="min-w-0 flex-1">
                                <div class="flex justify-between gap-2">
                    ...
                    @empty
                        <p class="text-sm text-gray-400 text-center py-8">No employer chats yet.</p>
                    @endforelse
                </div>
            </div>
        </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white overflow-hidden min-h-[620px] flex flex-col">
        @if($selectedConversation)
            <div class="p-4 border-b border-gray-200">
                <h3 class="font-semibold text-gray-900">{{ $headerTitle }}</h3>
                <p class="text-xs text-gray-500">{{ $headerSub }}</p>
            </div>
        ...
        @else
            <div class="flex-1 flex items-center justify-center text-center p-8">
                <div>
                    <h3 class="font-semibold text-gray-900 mb-2">Select a candidate, employer, or conversation</h3>
                    <p class="text-sm text-gray-500">Admin chats are private and not visible to other users.</p>
                </div>
            </div>
        @endif
    </div>
