...
// Sidebar badges are "unviewed/new" counts that clear as the employer reviews
// each thing — see App\Support\EmployerAlerts and the view events in the
// applicants / recruitment / payment controllers.
$counts = \App\Support\EmployerAlerts::sidebarCounts(auth()->user()->company, auth()->id());
...
$navItems = [
    ...
    ['route' => 'employer.manage-jobs', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>', 'label' => 'Manage Jobs'],
    ['route' => 'employer.applicants', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>', 'label' => 'All Applicants', 'badge' => $counts['applicants'], 'badge_class' => 'bg-red-500'],
    ['route' => 'employer.resumes', 'params' => ['saved' => 1], 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"/>', 'label' => 'Shortlisted Resumes'],
    ['route' => 'employer.recruitment-requests.index', 'active_prefix' => 'employer.recruitment-requests', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 8a2 2 0 100-4 2 2 0 000 4zm0 2c-2 0-4 1-4 3v1h8v-1c0-2-2-3-4-3z"/>', 'label' => 'Hiring Services', 'badge' => $counts['hiring']],
    ['route' => 'employer.resume-alerts', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>', 'label' => 'Resume Alerts'],
    ['route' => 'employer.chat', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/>', 'label' => 'Messages', 'badge' => $counts['messages']],
    ['route' => 'employer.payments', 'active_prefix' => 'employer.payments', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h2m4 0h4M5 6h14a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2z"/>', 'label' => 'Payments & Receipts', 'badge' => $counts['billing']],
];
@endphp
...
<div class="pt-4 mt-4 border-t border-[#E5E7EB]">
    ...
    <form method="POST" action="{{ route('auth.logout') }}">
        @csrf
        <button type="submit" class="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg text-red-600 hover:bg-red-50 transition">
